...
class OAuth extends RepositoryAbstract
{
    ...
    {
        $client->clean();
        if ((!$client->getName() or !$client->getSecret()) and !$client->getIdClient()) {
    ...
    public function getToken(OAuthToken $token)
    {
        ...
        $sql = 'SELECT TOKEN, EXPIRATION, FK_ID_CLIENT, FK_ID_USER FROM OAUTH_TOKENS WHERE TOKEN = :token AND EXPIRATION > SYSDATE()';
