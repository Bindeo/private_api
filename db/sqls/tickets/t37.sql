...
CREATE PROCEDURE InsertClients()
  BEGIN
    ...
    INSERT INTO PROCESSES(TYPE, ID_ELEMENT, CLIENT_TYPE, FK_ID_CLIENT, FK_ID_STATUS, NAME, SEARCH, CTRL_DATE)
      SELECT 'N', ID_FILE, CLIENT_TYPE, FK_ID_CLIENT, CONFIRMED + 1, NAME, REGEX_REPLACE('[^A-Za-z0-9]', ' ', NAME), CTRL_DATE FROM FILES WHERE MODE = 'N';

    INSERT INTO PROCESSES(TYPE, ID_ELEMENT, CLIENT_TYPE, FK_ID_CLIENT, FK_ID_STATUS, NAME, SEARCH, CTRL_DATE)
      SELECT 'S', B.ID_BULK_TRANSACTION, B.CLIENT_TYPE, B.FK_ID_CLIENT,
        CASE WHEN B.CLOSED = 0 THEN 1 WHEN B.CLOSED = 1 AND B.CONFIRMED = 0 THEN 2 ELSE 3 END, F.NAME, REGEX_REPLACE('[^A-Za-z0-9]', ' ', F.NAME), B.CTRL_DATE
